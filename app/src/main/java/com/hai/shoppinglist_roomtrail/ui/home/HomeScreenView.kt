...
fun CategoryItem(
    ...
    Card(
        ...
        border = BorderStroke(
            1.dp,
            if(selected) MaterialTheme.colorScheme.onPrimary.copy(.5f)
            else MaterialTheme.colorScheme.onSurface
        ),
        shape = Shapes.large,
        colors = CardDefaults.cardColors(
            containerColor = if (selected) MaterialTheme.colorScheme.primary.copy(.5f) else MaterialTheme.colorScheme.surface,
        )
    ){
        Row(
...
fun formatDate(date: Date): String = SimpleDateFormat("MM-dd-yyyy", Locale.getDefault())
    .format(date)
